...
def define_discriminator(img_shape):
    ...
    conv4 = Conv2D(filters=512, kernel_size=(4, 4), strides=1, padding='same',
                   kernel_initializer=RandomNormal(stddev=0.01),
                   bias_initializer=zeros_initializer(),
    ...
    conv5 = Conv2D(filters=512, kernel_size=(4, 4), strides=1, padding='same',
                   kernel_initializer=RandomNormal(stddev=0.01),
                   bias_initializer=zeros_initializer(),
    ...
    # define model
    model = Model(in_src_image, conv6)
    # compile model
